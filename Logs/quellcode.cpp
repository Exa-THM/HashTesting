...
unsigned long jenkins_one_at_a_time_hash(string);
...
unordered_map<string, HASH_FN> hashFuncs = { { "Jenkins One At A Time Hash 128", jenkins_one_at_a_time_hash } };
...
//Alle hash Funktionen in dieser Map werden dann später auch getestet

unordered_map<string, string> lexika = { { "AllWords", "../../../Lexika/FullLex.txt" }, { "OnlyB", "../../../Lexika/allWordsWithB.txt" }, { "Links", "../../../Lexika/somelinks.txt" } };
//unordered_map<string, string> lexika = { { "OnlyB", "../../../Lexika/allWordsWithB.txt" } };
...
LARGE_INTEGER measureAccessTimes(HASH_MAP map, string key, int times) {
	...
	ElapsedTime.QuadPart *= 1000000; //1000000
...
void runTestsFor(HASH_FN hashing_func, string hashName, string lexikon, string lexikonName) {
	...
	cout << "Lexikon: " << lexikon << "\n";
	cout << "Hashing Funct: " << hashName << "\n";
	...
	log << measureAccessTimes2(map, 4, 5, 200000); //200000
